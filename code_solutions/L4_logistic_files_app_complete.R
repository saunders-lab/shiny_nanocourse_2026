...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
          ...
          fileInput('file','Upload .csv file here:', buttonLabel = 'Cols: hrs & cells', accept = '.csv')
        ...
        mainPanel(
          ...
          tabsetPanel(
            tabPanel("Plot data", plotOutput("cells_plot", brush = 'plot_brush')),
...
server <- function(input, output) {
  ...
  ## TODO-3: Define a reactive that imports the input file with read_csv
  df_cells <- reactive({  
  ...
  df_cells_subset <- reactive({
    ...
    brushedPoints(df_cells(), input$plot_brush)
  ...
  mod_logis <- reactive({
    ...
    nls(formula = cells~SSlogis(input = hrs, Asym, xmid, scal), data = df_cells_subset())
